...
export const handler = async (event) => {
  ...
  try {
    ...
    const rows = await sql`
      SELECT html_output, title FROM documents
    ...
    if (!rows[0] || !rows[0].html_output) {
      return {
        statusCode: 404,
        headers: { "Content-Type": "text/html" },
        body: "<h1>404 — Document not found or has no HTML</h1>",
      };
    }

    // Inject a small print-helper banner at the top (won't show in print)
    const printBanner = `
<div id="report-ai-banner" style="position:fixed;top:0;left:0;right:0;z-index:99999;background:#1a2b5c;color:white;padding:10px 20px;font-family:system-ui,sans-serif;font-size:14px;display:flex;justify-content:space-between;align-items:center;print-color-adjust:exact;">
  ...
  <div>
    <button onclick="document.getElementById('report-ai-banner').style.display='none';window.print()" style="background:white;color:#1a2b5c;border:none;padding:8px 20px;border-radius:4px;cursor:pointer;font-weight:600;font-size:14px;">
      Skriv ut / Spara som PDF
    </button>
    <button onclick="document.getElementById('report-ai-banner').style.display='none'" style="background:transparent;color:white;border:1px solid rgba(255,255,255,0.3);padding:8px 16px;border-radius:4px;cursor:pointer;margin-left:8px;font-size:14px;">
      Dölj
    </button>
  </div>
</div>
<style>
  @media print { #report-ai-banner { display: none !important; } }
  ...
  @media print { body { padding-top: 0 !important; } }
    ...
    return {
      statusCode: 500,
      headers: { "Content-Type": "text/html" },
      body: `<h1>500 — Server error</h1><p>${escapeHtml(e.message)}</p>`,
    };
  }
};
